...
final class PostgresEventStore implements PdoEventStore, TransactionalEventStore
{
    ...
    public function appendTo(StreamName $streamName, Iterator $streamEvents): void
    {
        ...
        $sql = 'INSERT INTO ' . $this->quoteIdent($tableName) . ' (' . implode(', ', $columnNames) . ') VALUES ' . $allPlaces; // phpcs:ignore
...
EOT;
        ...
        $statement = $this->connection->prepare($sql);
        try {
            $statement->execute(['streamName' => (string)$streamName]);
        } catch (PDOException) {
        }
...
SQL;
    ...
    {
        if (true === $this->disableTransactionHandling) {
            return;
        }
    ...
    private function createWhereClause(?MetadataMatcher $metadataMatcher): array
    {
        ...
        foreach ($metadataMatcher->data() as $key => $match) {
            ...
            if ($fieldType === FieldType::METADATA) {
                if (is_bool($value)) {
                    $where[] = "metadata->>'$field' $operatorString '" . var_export($value, true) . "' $operatorStringEnd"; // phpcs:ignore
                    continue;
                }
                if (is_int($value)) {
    ...
    {
        $realStreamName = (string)$stream->streamName;
...
EOT;
        ...
        try {
            $statement->execute([(string)$streamName]);
        } catch (PDOException) {
        }
